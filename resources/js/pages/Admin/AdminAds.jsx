...
export const AdminAds = ({ token }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">Kelola Iklan Baris</h2>
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 sm:mt-1">Lakukan kurasi, moderasi, dan kelola visibilitas iklan di marketplace.</p>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">Info iklan</th>
              ...
              {loading ? (
                ...
                <tr key={p.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${actionLoading === p.id ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Lokasi: {p.location}</p>
                  </td>
