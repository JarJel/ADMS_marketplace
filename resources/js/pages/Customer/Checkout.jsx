...
export default function Checkout({ user, token, onNavigate, onLogout, darkMode = true, setDarkMode, cartCount, wishlistCount, notifications }) {
    ...
    const handleRedirectWhatsApp = (orderResult) => {
        ...
        if (firstItem?.isPackage) {
            phone = '6281121211933'; // Default platform admin WA
    ...
    const handleProcessPayment = async (e) => {
        ...
        const guestItems = JSON.parse(localStorage.getItem('adms_guest_cart') || '[]');
        const checkedKeys = checkoutItems.map(item => (item.product_id || item.product?.id || item.id)?.toString());
        const remainingCart = guestItems.filter(g => {
            const key = (g.product_id || g.product?.id || g.id)?.toString();
    ...
    return (
        <div className={`min-h-screen font-sans transition-colors duration-300 relative overflow-hidden ${darkMode ? 'bg-[#071922] text-slate-100' : 'bg-slate-50 text-slate-900'
            }`}>
            {/* Ambient Glows */}
            <div className={`absolute top-0 right-0 w-[600px] h-[600px] rounded-full blur-[150px] pointer-events-none -translate-y-1/2 translate-x-1/4 ${darkMode ? 'bg-[#FFBF00]/10' : 'bg-[#FFBF00]/20'
                }`}></div>
            <div className={`absolute bottom-0 left-0 w-[600px] h-[600px] rounded-full blur-[150px] pointer-events-none translate-y-1/3 -translate-x-1/4 ${darkMode ? 'bg-[#0F3040]/40' : 'bg-emerald-300/20'
                }`}></div>
            ...
            <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <button
                        ...
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold border transition-all cursor-pointer ${darkMode
                ...
                {/* Page Title */}
                <div className="mb-8">
                    <h1 className={`text-3xl sm:text-4xl font-black tracking-tight leading-tight ${darkMode ? 'text-white' : 'text-slate-900'
                        }`}>
                ...
                {checkoutItems.length === 0 ? (
                    <div className={`rounded-3xl border p-12 text-center max-w-xl mx-auto backdrop-blur-xl ${darkMode ? 'bg-[#0F3040] border-[#174256]' : 'bg-white border-slate-200'
                        }`}>
                    ...
                    <form onSubmit={handleProcessPayment} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <div className="lg:col-span-7 space-y-6">
                            ...
                            <div className={`rounded-3xl border p-6 backdrop-blur-xl ${darkMode ? 'bg-[#0F3040] border-[#174256]' : 'bg-white border-slate-200 shadow-sm'
                                }`}>
                                <h3 className={`font-bold text-base mb-4 flex items-center gap-2 ${darkMode ? 'text-white' : 'text-slate-900'
                                    }`}>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full px-3.5 py-2.5 rounded-xl border focus:outline-none focus:ring-1 ${darkMode ? 'bg-[#071922] border-[#174256] text-white focus:border-[#FFBF00]' : 'bg-slate-50 border-slate-300 text-slate-800 focus:border-[#FFBF00]'
                                                }`}
                                        />
                                    </div>
                                    <div>
                            ...
                            <div className={`rounded-3xl border p-6 backdrop-blur-xl ${darkMode ? 'bg-[#0F3040] border-[#174256]' : 'bg-white border-slate-200 shadow-sm'
                                ...
                                <div className="space-y-3 mb-6">
                                    ...
                                    <label
                                        ...
                                        className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${paymentMethod === 'qris'
                                            ? `bg-[#FFBF00]/10 border-[#FFBF00] ${darkMode ? 'text-white' : 'text-slate-900'} shadow-md shadow-[#FFBF00]/10`
                                    ...
                                    {/* Option 2: Mandiri */}
                                    <label
                                        onClick={() => setPaymentMethod('transfer_mandiri')}
                                        className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${paymentMethod === 'transfer_mandiri'
                                            ? `bg-[#FFBF00]/10 border-[#FFBF00] ${darkMode ? 'text-white' : 'text-slate-900'} shadow-md shadow-[#FFBF00]/10`
                                            : (darkMode ? 'bg-[#071922]/60 border-[#174256] text-slate-300 hover:border-slate-700' : 'bg-slate-50 border-slate-200 text-slate-700')
                                            }`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-700 to-indigo-800 flex items-center justify-center text-[#FFBF00] shrink-0 shadow-md">
                                                <Building2 className="w-5 h-5" />
                                            </div>
                                            <div>
                                                <strong className="block text-xs font-bold">Transfer Bank Mandiri</strong>
                                ...
                                {paymentMethod === 'qris' && (
                                    <div className={`p-4 rounded-2xl border text-center space-y-3 ${darkMode ? 'bg-[#071922] border-[#174256]' : 'bg-amber-50/50 border-amber-200'
                                        }`}>
                                ...
                                {paymentMethod === 'transfer_mandiri' && (
                                    <div className={`p-4 rounded-2xl border space-y-3 text-xs ${darkMode ? 'bg-[#071922] border-[#174256]' : 'bg-slate-100 border-slate-200'
                                        }`}>
                                        <div className="flex items-center justify-between p-3 bg-[#0F3040] border border-[#174256] rounded-xl">
                                            <div>
                                                <span className="text-[10px] text-slate-400 block uppercase font-bold">{paymentConfig?.mandiri?.name ?? 'Bank Mandiri'}</span>
                                                <strong className="text-base text-blue-400 font-mono">{paymentConfig?.mandiri?.number ?? '1310025578545'}</strong>
                                                <span className="block text-[10px] text-slate-400">a.n. {paymentConfig?.mandiri?.account_name ?? 'PT ARMADA DIGITAL MARKETING SYARIAH'}</span>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => handleCopy(paymentConfig?.mandiri?.number?.replace(/-/g, '') ?? '1310025578545', 'mandiri')}
                                            ...
                                            >
                                                {copiedText === 'mandiri' ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                                                <span>{copiedText === 'mandiri' ? 'Tersalin' : 'Salin Rek'}</span>
                                            </button>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
                            <div className={`rounded-3xl border p-6 shadow-2xl relative overflow-hidden backdrop-blur-xl ${darkMode
                                ? 'bg-gradient-to-br from-[#0F3040] to-[#071922] border-[#174256] shadow-black/60'
                                : 'bg-white border-slate-200 shadow-xl'
                                }`}>
                                <h3 className={`font-black text-lg mb-4 flex items-center gap-2 ${darkMode ? 'text-white' : 'text-slate-900'
                                    }`}>
            ...
            {isSuccessModalOpen && createdOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
                    <div className={`w-full max-w-lg rounded-3xl border p-6 sm:p-8 shadow-2xl text-center relative overflow-hidden animate-in zoom-in-95 ${darkMode ? 'bg-[#0F3040] border-[#174256] text-white' : 'bg-white border-slate-200 text-slate-900'
                        }`}>
